...
class CategoricalModel(nn.Module):
    def __init__(self, D_m_party_text,D_m_party_audio,D_m_party_video, D_m_context, D_g, D_p, D_e, D_h, n_classes=7, D_a=100, dropout_rec=0.5, dropout=0.5):
        ...
        # self.prior_hidden1 = nn.Linear(3*D_m_party_text,2)
        # self.prior_hidden2 = nn.Linear(2*D_h,D_h)
        # self.prior_hidden3 = nn.Linear(D_h,2)
    ...
    def forward(self, U_t, U_a, U_v, U_context, U_t_, P_t, qmask, umask, softmax_loss):
        ...
        # siamese_prob = 1 - self.cos(U_t,P_t)
        # print(siamese_prob.shape)
        # siamese_prob_back = siamese_prob
        siamese_prob = F.softmax(siam_f1,2)
        emotion_shift_prob = 1-siamese_prob
        # print(siamese_prob.shape)
        siamese_prob_back = torch.zeros(siamese_prob.shape).type(siamese_prob.type())
        ...
        emotions_b_t, alpha_b_t = self.multilogue_net_r_t(rev_U_t, rev_U_context, rev_qmask, siamese_prob_back)
        ...
        emotions_b_a, alpha_b_a = self.multilogue_net_r_a(rev_U_a, rev_U_context, rev_qmask, siamese_prob_back)
        ...
        emotions_b_v, alpha_b_v = self.multilogue_net_r_v(rev_U_v, rev_U_context, rev_qmask, siamese_prob_back)
