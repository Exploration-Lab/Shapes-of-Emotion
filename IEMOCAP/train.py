...
def get_IEMOCAP_loaders(path, batch_size=128, valid = 0.1, num_workers=0, pin_memory=False):
    trainset = IEMOCAPCategorical(path=path,  train= True, bert_vectors=PATH.BERT_VECTORS, siamese_vectors = PATH.SBERT_VECTORS)
    ...
    testset = IEMOCAPCategorical(path=path, train=False, bert_vectors=PATH.BERT_VECTORS, siamese_vectors = PATH.SBERT_VECTORS)
...
if __name__ == '__main__':
    ...
    D_m_text, D_m_audio, D_m_video, D_m_context = 768, 100, 512, 768
    ...
    train_loader, valid_loader, test_loader = get_IEMOCAP_loaders(PATH.CATEGORICAL_DATA, batch_size=batch_size, num_workers=0)

    best_loss, best_label, best_pred, best_mask, best_fscore = None, None, None, None, None

    
    # Training loop
    for e in tqdm(range(n_epochs), desc = 'IEMOCAP Categorical'):
        ...
        writer.add_scalar("Train Loss - IEMOCAP Sentiment", train_loss, e)
        writer.add_scalar("Val Loss - IEMOCAP Sentiment", val_loss, e)
