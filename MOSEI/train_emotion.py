...
if __name__ == '__main__':
    ...
    parser.add_argument('--emotion', type=str, required=True,  metavar='emotion', help='emotion to train')
...
# 0.08276465441819772
    ...
    if emotion=='happiness': 
        loss_weights = torch.FloatTensor([1/0.4648, 1/0.5352])
        emotion_label = 0
    elif emotion=='sadness': 
        loss_weights = torch.FloatTensor([1/0.7369, 1/0.2631])
        emotion_label = 1
    elif emotion=='anger': 
        loss_weights = torch.FloatTensor([1/0.7877, 1/0.2123])
        emotion_label = 2
    elif emotion=='surprise': 
        loss_weights = torch.FloatTensor([1/0.8987, 1/0.1013])
        emotion_label = 3
    elif emotion=='disgust': 
        loss_weights = torch.FloatTensor([1/0.8222, 1/0.1778])
        emotion_label = 4
    elif emotion=='fear': 
        loss_weights = torch.FloatTensor([1/0.9172, 1/0.0828])
        emotion_label = 5
    ...
    for e in tqdm(range(n_epochs), desc = 'MOSEI Categorical'):
        ...
        writer.add_scalar("Val Loss - MOSEI Emotion (" + str(emotion) + ")", val_loss, e)
